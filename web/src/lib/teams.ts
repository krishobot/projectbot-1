...
export type TeamSkill = { name: string; source: "astack" | "tbrain" | "external" };
...
const SKILL_LINE = /\| `(\/?[a-z0-9_-]+)` \| (astack|tbrain|external|—) \|/g;
...
function parseFile(filename: string): Team {
  ...
  const astackSkills: string[] = [];
  const tbrainSkills: string[] = [];
  for (const m of content.matchAll(SKILL_LINE)) {
    const [, skillName, source] = m;
    if (source === "astack") astackSkills.push(skillName);
    else if (source === "tbrain") tbrainSkills.push(skillName);
  }
  ...
  return { id, number, name, charter, body: content, astackSkills, tbrainSkills, brainPages };
}
