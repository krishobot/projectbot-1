...
Thirteen teams. Every skill in upstream GStack (the role layer) and GBrain (the memory layer, called **tbrain** when bundled with our Supabase backend) is owned by exactly one team — single owner, multiple readers, like the GBrain "single-writer-per-file" rule applied to roles.

## One-page map

| # | Team | Owns the decision | Lead GStack skills | Lead GBrain skills |
|---|------|-------------------|--------------------|--------------------|
...
| 06 | **Release / DevOps** | Get it shipped, watch it land | `/ship` `/land-and-deploy` `/canary` `/landing-report` `/document-release` `/setup-deploy` `/supabase` `/gstack-upgrade` | `migrate` `publish` |
| 07 | **DevEx** | Developer experience, environment, hosts | `/plan-devex-review` `/devex-review` `/setup` `/setup-browser-cookies` `/setup-gbrain` `/open-gstack-browser` `/connect-chrome` `/openclaw` `/pair-agent` | — |
...
| 13 | **Data / Analytics** | Measurement, retrieval quality, eval harnesses | `/benchmark` `/benchmark-models` | `query` `reports` (+ BrainBench in `gbrain-evals` sibling repo) |

`AGENTS.md` and `CLAUDE.md` in each cloned repo are the authoritative skill catalogs. This table is the org's view of where each skill works.
...
These are inherited from Tan's GStack/GBrain ethos and apply org-wide:

1. **Markdown is source of truth.** Decisions, plans, and entity records live in markdown files in `~/gbrain/` (or your configured brain dir). Not in chat history, not in tickets.
2. **Single writer per file.** One team owns each brain page; others read. If two teams need to write, split the page.
3. **Append-only evidence trails.** Editable summary at top, immutable timeline below. Per the GBrain "Compiled Truth + Timeline" pattern.
